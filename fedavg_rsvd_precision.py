...
def fedavg_rsvd_precision(
            ...
            for layer in self.model_params.tensors:
                if rank > 0 and layer.shape == COMP_SHAPES.get(dataset, None):
                    try:
                        ...
                        res.append((U.astype(nptype), S.astype(nptype), Vt.astype(nptype)))
                    except:
                        res.append(layer.astype(torchtype))
                else:
                    res.append(layer.to(torchtype))
    ...
    rayleaf.run_experiment(
        ...
        output_dir= Path(output_dir, dataset, "fedavg", f"r{str(rank).zfill(4)}-precision{precision}-{num_clients}clients-{clients_per_round}cpr-{client_lr}lr-{num_epochs}epochs-{num_rounds}rounds"),
        model = model,
